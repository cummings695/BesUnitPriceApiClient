//
// HttpValidationProblemDetails.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation
#if canImport(AnyCodable)
import AnyCodable
#endif

public final class HttpValidationProblemDetails: Codable, JSONEncodable, Hashable {

    public var type: String?
    public var title: String?
    public var status: Int?
    public var detail: String?
    public var instance: String?
    public var errors: [String: [String]]?

    public init(type: String? = nil, title: String? = nil, status: Int? = nil, detail: String? = nil, instance: String? = nil, errors: [String: [String]]? = nil) {
        self.type = type
        self.title = title
        self.status = status
        self.detail = detail
        self.instance = instance
        self.errors = errors
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case type
        case title
        case status
        case detail
        case instance
        case errors
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(type, forKey: .type)
        try container.encodeIfPresent(title, forKey: .title)
        try container.encodeIfPresent(status, forKey: .status)
        try container.encodeIfPresent(detail, forKey: .detail)
        try container.encodeIfPresent(instance, forKey: .instance)
        try container.encodeIfPresent(errors, forKey: .errors)
    }

    public static func == (lhs: HttpValidationProblemDetails, rhs: HttpValidationProblemDetails) -> Bool {
        lhs.type == rhs.type &&
        lhs.title == rhs.title &&
        lhs.status == rhs.status &&
        lhs.detail == rhs.detail &&
        lhs.instance == rhs.instance &&
        lhs.errors == rhs.errors
        
    }

    public func hash(into hasher: inout Hasher) {
        hasher.combine(type?.hashValue)
        hasher.combine(title?.hashValue)
        hasher.combine(status?.hashValue)
        hasher.combine(detail?.hashValue)
        hasher.combine(instance?.hashValue)
        hasher.combine(errors?.hashValue)
        
    }
}

